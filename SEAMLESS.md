...
5. **真實世界切換自動讓路**:若 join 過程中後端送出 Respawn(例如同步插件把玩家
   ...
   後端送來的一律是未解碼封包)。
6. commit 後 proxy 代客戶端向目標伺服器送出 **Player Loaded**(1.21.4+):客戶端沒有真的
   重載世界,不會自己送這個封包;不補的話伺服器會凍結玩家移動直到約 3 秒的逾時。
   若 join 過程有真實 Respawn 則不代送,由客戶端重載後自行回報。
